...
function CustomSideBar() {
	...
	useEffect(() => {
		axios
			.get(
				`${process.env.REACT_APP_API_URL}getAccountById/` +
					currentUser.id
			)
			.then((res) => {
	...
	switch (currentUser.role) {
		...
		case "Section Department":
			let isAssemble = JSON.parse(
				localStorage["currentSectionInfo"]
			).isAssemble;
			if (isAssemble) {
						...
						<MenuItem onClick={handleOpenProfile}>
							<Avatar
								src={
									...
									currentUserBar.avatarUrl
								}
								sx={{ width: 36, height: 36 }}
							/>
							&emsp;
							{currentUserBar.name}&emsp;
							<div style={{ fontSize: "26px" }}>|</div>&emsp;
							{roles.map((item) => {
