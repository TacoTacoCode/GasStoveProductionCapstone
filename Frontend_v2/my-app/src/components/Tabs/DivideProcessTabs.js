...
function DivideProcessTabs() {
	...
	useEffect(() => {
		axios
			.post(`${process.env.REACT_APP_API_URL}distribute`, {
				process: curProcess,
				ProcessAmmounts: arr,
			})
			.then((response) => {
				...
				response.data.map((e) => {
					...
					e.processDetails.map((el) => {
						details.push({
							...el,
							componentName:
								listComponent[el.sectionId].componentName,
							componentImg:
								listComponent[el.sectionId].componentImg,
						});
					});
					datas.push(details);
				});
	...
	const columns = [
		...
		},
		{
			...
			editable: "false",
			cellStyle: { fontFamily: "Muli", width: "15%" },
			align: "center",
			render: (rowData) =>
			...
			editComponent: (props) => (
				<LocalizationProvider dateAdapter={AdapterDateFns}>
					<DatePicker
						...
						minDate={
							listProcess[toggleState - 1].createdDate == null
								? null
								: new Date(
										listProcess[toggleState - 1].createdDate
								  )
						}
						maxDate={
							listProcess[toggleState - 1].expiryDate == null
								? null
								: new Date(
										listProcess[toggleState - 1].expiryDate
								  )
						}
						onChange={(date) => {
							let dateFm = moment(date).format("MM/DD/YYYY");
	...
	return (
		<>
			<div style={{ marginBottom: "1%", marginTop: "1%" }}>
				<ImportExcelButton type="submit" onClick={(e) => handleSave(e)}>
					Submit
				</ImportExcelButton>
			</div>
							...
							<div className="number">
								<div className="divprocess">
									<CssTextField
										...
										label={"Total Amount"}
										value={
											listProcess[index] === undefined
												? 0
												: listProcess[index].totalAmount
										}
										onChange={(e) =>
								...
								<div className="divprocess">
									...
									>
										<DatePicker
											...
											onChange={(date) =>
												updateExpectedFinishDateChanged(
													index,
													date
												)
											}
