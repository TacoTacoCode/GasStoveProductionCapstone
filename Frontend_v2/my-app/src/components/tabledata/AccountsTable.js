...
export const Table = (props) => {
	...
	listAccount.forEach((item) => {
		if (item.roleId != "ADM") array.push(item);
	}, []);
	...
	const columns = [
			...
			render: (rowData) =>
				rowData.avatarUrl != null ? (
					<Avatar
						...
						src={
							rowData.avatarUrl.includes("https")
								? rowData.avatarUrl
		...
		},
		{
			...
			cellStyle: { fontFamily: "Muli", paddingRight: "3%" },
			align: "center",
			render: (rowData) => (rowData.gender == true ? "Male" : "Female"),
		},
		{
			title: "Date of Birth",
			field: "dateOfBirth",
			...
			render: (rowData) =>
				rowData.dateOfBirth == null
					? `Chưa cập nhật`
					: moment(rowData.dateOfBirth).format("MM/DD/YYYY"),
		},
		{
			title: "Status",
			field: "status",
			...
			<MaterialTable
				...
				actions={[
					...
					(rowData) => ({
						...
						disabled:
							rowData.roleId == "CUS" || rowData.roleId == "ADM",
					}),
				]}
				options={{
