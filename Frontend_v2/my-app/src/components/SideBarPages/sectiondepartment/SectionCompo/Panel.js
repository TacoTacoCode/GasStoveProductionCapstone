...
const TransportFlow = () => {
	...
	useEffect(() => {
		axios
			.get(
				`${process.env.REACT_APP_API_URL}getMateByCompoId/${secInfo.componentId}`
			)
			.then((response) => {
	...
	const generateSubmitData = () => {
		if (values[0].length == 0) {
			...
			values.map((value, index) => {
				...
				tmp.map((ele) => {
					if (ele.amount != 0 && ele.amount != null) {
						let obj = arr.find(
							(e) =>
								e.processDetailId === ele.processDetailId &&
								e.itemId === ele.itemId
						);
						if (obj !== undefined) {
							...
							if (obj.amount > obj.maxAmount) {
								alert(
									`Total amount of ${obj.itemName} exceed required (${obj.maxAmount})`
								);
							}
						} else {
							arr.push(ele);
			...
			arr = arr.map((e) => {
				return { ...e, itemId: e.itemId + "M", exportedAmount: 0 };
			});
			return arr;
		}
	};
	...
	return (
		<div className="flow" style={{ margin: "50px 30%" }}>
			...
			<div>
				<ImportExcelButton onClick={() => addPoint()}>
					Add
				</ImportExcelButton>
				{values.length > 0 ? (
					<ImportExcelButton onClick={() => submitAll()}>
						Submit
					</ImportExcelButton>
				) : (
					<ImportExcelButton
