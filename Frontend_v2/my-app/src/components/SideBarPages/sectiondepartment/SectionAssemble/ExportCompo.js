	...
	const columns = [
				...
				<div style={{ width: "100%", paddingLeft: "10%" }}>
					...
					>
						{tableData[rowData.tableData.id].listCompos.length > 0
							? tableData[rowData.tableData.id].listCompos.map(
									(el) => (
										<MenuItem
											value={el.name}
											key={el.id}
		...
		},
		{
			...
			render: (rowData) => (
				<Tooltip
					title={`Maximum is ${
						tableData[rowData.tableData.id].maxAmount
					}`}
				>
					<TextField
						variant="outlined"
						...
						InputProps={{
							...
							inputProps: {
								min: 0,
								max: tableData[rowData.tableData.id].maxAmount,
							},
						}}
						onChange={(e) =>
							handleChangeAmount(
	...
	const generateSubmitData = () => {
		if (tableData.length == 0) {
			...
			!tableData.every(
				(value) =>
					value.amount > 0 &&
					value.processDetailId.toString().length > 0 &&
			...
			tableData.map((value) => {
				let obj = arr.find(
					(e) =>
						e.processDetailId === value.processDetailId &&
						e.itemId === value.itemId
				);
				if (obj !== undefined) {
					obj.amount += value.amount;
					if (obj.amount > obj.maxAmount) {
						alert(
							`Total amount of ${obj.componentName} exceed required (${obj.maxAmount})`
						);
					}
				} else {
					arr.push({
			...
			{datas.length > 0 ? (
				<MaterialTable
					title={
						<MyNewTitle
							variant="h6"
							text="Make a request for component"
						/>
					}
					data={tableData}
					columns={columns}
